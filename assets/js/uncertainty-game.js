...
(function () {
  ...
  function resetAll() {
    S = {
      phase: 'intro', score: 0, round: 0, streak: 0, trust: 100,
      incidents: 0, maxIncidents: 3, missed: 0, assists: 0, avoided: 0,
      best: Number(localStorage.getItem('uqGameBest') || 0),
      cur: null, timeLeft: 0, timeMax: 0, revealTO: null, cw: 480, ch: 300
    };
  }
  resetAll();
  ...
  function drawScene(revealing) {
    ...
    var spread = 12 + s.noise * Math.min(w, h) * 0.42;
    ...
    if (revealing) {
      ...
      ctx.beginPath(); ctx.arc(t.x, t.y, 27, 0, 7); ctx.stroke();
      ctx.fillStyle = col; ctx.font = '600 13px system-ui, sans-serif';
      ctx.fillText('actual target', t.x - 40, t.y - 36);
    }
  }
  ...
  function makeScenario(round) {
    var diff = Math.min(1, round / 16);
    var noise = clamp(0.14 + Math.random() * 0.66 + diff * 0.14, 0.1, 0.97);
    var pCorrect = clamp(1 - noise * 0.92 - Math.random() * 0.08 + 0.04, 0.02, 0.98);
    var correct = Math.random() < pCorrect;
    var conf;
    if (Math.random() < 0.22 + diff * 0.4) {
  ...
  // ---- HUD ----
  function setBot(state) {
    els.bot.className = 'game-bot is-' + state;
  }
  function syncHUD() {
  ...
  function newRound() {
    ...
    S.timeMax = reduce ? Infinity : Math.max(4, 7 - S.round * 0.2);
  ...
  function gameOver() {
    ...
    if (S.score > S.best) {
      S.best = S.score;
      localStorage.setItem('uqGameBest', S.best);
      els.best.textContent = S.best;
    }
    showOver();
  }
  ...
  function showIntro() {
    ...
    els.overlay.innerHTML =
      ...
      '<h2>Act or Hold</h2>' +
      '<p>You are the safety layer of an assistive robot. Each round shows a noisy sensor estimate and a reported confidence. Choose <b>ACT</b> to assist or <b>HOLD</b> to wait. Acting on a wrong estimate causes a safety incident. Three incidents ends the run.</p>' +
      '<ul class="ov-rules"><li><b>ACT</b> and correct: assist delivered</li><li><b>ACT</b> and wrong: safety incident</li><li><b>HOLD</b> and it was wrong: incident avoided</li><li><b>HOLD</b> and it was right: missed assist</li></ul>' +
      '<button class="game-btn act big" id="g-start">Start <kbd>↵</kbd></button>' +
      '<p class="ov-fine">Keys: A to act, H to hold. Best score is saved on this device.</p>' +
      '</div>';
  ...
  function showOver() {
    ...
    els.overlay.innerHTML =
      ...
      stat('Missed', S.missed) + stat('Incidents', S.incidents) +
      '</div>' +
      '<button class="game-btn act big" id="g-again">Play again <kbd>↵</kbd></button>' +
      '</div>';
